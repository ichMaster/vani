...
Delivery is organized into three product versions with numbered phases (see the roadmap); cross-references use the form `vN Pk` (e.g. `v2 P2` is Version 2, Phase 2).
...
- **Versions 0–1 and Version 2 Phase 1:** a single Python process — a text TUI, then voice on the developer's machine. State is local JSON.
- **Version 2 Phase 2:** a server backend (WebSocket) and MongoDB.
- **Version 2 Phase 3:** the Echo Pyramid device as a terminal.
...
vani/                      # repo root; .venv/ virtualenv, pyproject.toml (deps + pytest/ruff config)
  src/
    app/
      tui_main.py          # entry point Versions 0-1, v2 P1 (local TUI)
      server_main.py       # entry point v2 P2+ (server)
    ...
    state/
      ...
      json_store.py        # implementation, through v2 P1
      mongo_store.py       # implementation, v2 P2+
    io/                    # v2 P1: asr (Whisper), tts (Piper), barge-in
    server/                # v2 P2: FastAPI + WebSocket, sessions, protocol, filler
    device/                # v2 P3: Echo Pyramid integration (xiaozhi firmware, config)
...
flowchart TB
  subgraph ENTRY["Entry points"]
    TUIM["app/tui_main (Versions 0-1, v2 P1)"]
    SRVM["app/server_main (v2 P2+)"]
  end
  PL["planner"]
  LLM["llm: Haiku/Opus"]
  GU["guardian"]
  ...
  BG["background (async)"]
  ST["state: repository"]
  J["json_store (through v2 P1)"]
  MG["mongo_store (v2 P2+)"]
  IO["io: ASR/TTS (v2 P1)"]
  SRV["server: WS, sessions, filler (v2 P2)"]
  DEV["device: Echo Pyramid (v2 P3)"]
  TUIM --> PL
  SRVM --> SRV
  SRV --> PL
  PL --> LLM
...
- **delivery** — style profile, envelope, fluctuation (textual from v1 P5, prosodic from v2 P1).
...
- **state** — a repository with json (through v2 P1) and mongo (v2 P2+) implementations.
- **io** — ASR/TTS (v2 P1).
- **server** — WebSocket, sessions, device protocol, filler (v2 P2).
- **device** — Echo Pyramid: firmware and config (v2 P3).
...
The same logical documents at every stage; only the implementation behind the repository changes: JSON files (through v2 P1) -> MongoDB collections (v2 P2+).
...
### 5.1 A Turn in the Local Build (through Version 2 Phase 1)

```mermaid
flowchart TB
  IN["Input (tui; v2 P1 — voice)"] --> P1["Perception — Haiku"]
  ...
  OP --> GU
  GU --> OUT["Delivery to output (v2 P1: TTS)"]
...
### 5.2 A Turn via Server and Device (Version 2 Phases 2-3)
...
### 6.1 Local (through Version 2 Phase 1)
...
### 6.2 Client-Server with Device (Version 2 Phases 2-3)
...
asyncio in a single process: the main turn loop (the fast path) and a background task (`background`) over a material queue; the background does not block the response and is triggered selectively. The Guardian is synchronous in the main loop. On the server (v2 P2), many sessions are served by the same brain.

---

## 8. Technology Stack by Stage

- **Versions 0–1 and Version 2 Phase 1:** Python in a `.venv` virtualenv with `pyproject.toml`; `pytest` (tests) and `ruff` (lint/format); Textual (TUI); Anthropic SDK (Haiku/Opus); a local LLM fallback (Gemma/Qwen via Ollama/llama.cpp) for offline turns; skyfield (astro); asyncio; local JSON; Whisper (ASR) and Piper-Ukrainian (TTS) from v2 P1.
- **Version 2 Phase 2:** FastAPI + WebSocket; MongoDB.
- **Version 2 Phase 3:** AtomS3R with xiaozhi firmware; the Opus codec; audio streaming.
...
- **Versioned and migratable.** Every document carries `schema_version`; the repository migrates on read (old -> current), so field evolution and the JSON -> Mongo move (v2 P2) never break existing state.
- **JSON-serializable.** One representation serves local JSON, Mongo documents, and the v2 API payloads — which is what keeps the API migration cheap (Section 10). The schemas are exported as JSON Schema (draft 2020-12) under [`architecture/schemas/`](schemas/) — one file per persisted document plus the `PerceptionResult` and `TurnPlan` contracts; see its README for conventions.

---

## 10. Brain/Transport Boundary and API-Readiness

V3 turns the local app into an API service (v2 P2). To make that a wrapper and not a rewrite, the boundary is drawn from V1:
...
- **Adapters, not forks.** Every surface is a thin adapter over the same engine: the TUI (V1, text), the voice loop (v2 P1, ASR <-> TTS), and the FastAPI + WebSocket server (v2 P2, Opus frames). Adding the API adds an adapter; the brain is untouched.
...
- **Cached prefix (stable / slow-changing):** the system block — the compiled identity block from the canon (a minimal placeholder in V1, the full canon from v1 P1) and the daily temperament block (from v1 P2). Sent with Anthropic prompt caching: the identity part is stable, the temperament part refreshes once per day.
...
- **Low-confidence perception/intent:** re-ask rather than act on a guess (uses the confidence attribute, v0 P3).
- **Repository write failure:** atomic snapshot writes (v1 P6) so a crash never leaves half-written state.
- **Background pass failure:** isolated from the main turn — it may drop a cycle but never blocks or breaks the response.
- **Later phases:** barge-in (interrupt TTS, cancel the in-flight Opus call) arrives with voice (v2 P1); MCP/tool errors (report honestly, never fabricate) arrive with tools.
...
- **Sensitive data at the repository layer:** the user's birth data and the portrait's vulnerability hypotheses are encrypted at rest inside `json_store` behind the `Repository` interface, redacted in telemetry, and deletable on demand. Designed from v0 P0 so later layers inherit it.
- **Local-only:** state stays on the machine; nothing is exported. With local ASR/TTS (v2 P1) the voice never leaves the perimeter — only text would.
- **Untrusted input:** content from tools, MCP, or the web is treated as data, never instructions; commands found in it are not executed without explicit user confirmation. The full boundary lands with tools and the Guardian (v1 P4); the principle holds from the start.
...
- **Environment:** a `.venv` virtualenv per checkout; dependencies, build, and tool config in `pyproject.toml`. Stood up at v0 P0.
